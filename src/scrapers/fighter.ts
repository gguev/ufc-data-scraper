...
export async function getFighter(slug: string): Promise<Fighter> {
  ...
  try {
    ...
    if (!info) {
      throw new ScrapingError("Failed to parse fighter info from page", {
        url,
      });
    }

    if (!stats) {
      throw new ScrapingError("Failed to parse fighter stats from page", {
        url,
      });
    }
...
function parseRank(rankTag: string): Rank {
  if (rankTag.toLowerCase().includes("title holder")) {
    return "C";
  }

  const rankMatch = rankTag.match(RANK_REGEX);
  if (rankMatch?.[1]) {
...
function parseFighterInfo($: CheerioAPI) {
  try {
    const getBioText = (selector: string) =>
      $(`.c-bio__label:contains("${selector}")`)
        .next(".c-bio__text")
        .text()
        .trim();

    const rankTag = $(".hero-profile__tag").text().trim();
    ...
    return {
      ...
      nickname: $(".hero-profile__nickname")
        ...
        .trim()
        .replace(/^"|"$/g, ""),
      status: getBioText("Status") as "Active" | "Retired" | "Not Fighting",
      rank: parseRank(rankTag),
...
function parseFighterStats($: CheerioAPI) {
  try {
    const getStatValue = (selector: string) =>
    ...
    const firstRoundFinishes =
      Number(
        $(".athlete-stats__stat")
          // biome-ignore lint/suspicious/noExplicitAny: Cheerio filter requires any for compatibility
    ...
    const sigStrikeLandedPercent = Number.parseFloat(
    ...
    const takedownsLandedPercent = Number.parseFloat(
...
export function parseRecord(txt: string): {
  ...
  const record = txt.match(RECORD_REGEX);
  if (!record) {
    return {
      wins: 0,
      losses: 0,
  ...
  const [, winsStr, lossesStr, drawsStr] = record;
  return {
    wins: +(winsStr || 0),
    losses: +(lossesStr || 0),
    draws: +(drawsStr || 0),
