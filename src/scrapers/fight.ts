  ...
  try {
    ...
    for (const src of frames) {
      ...
      const data = await frame.evaluate(() => {
        const redFightOverview = document.querySelectorAll('.red .c-stat-metric-compare__number')

        const blueFightOverview = document.querySelectorAll('.blue .c-stat-metric-compare__number')

        const redSigHead = document.querySelectorAll('.red #e-stat-body_x5F__x5F_head_value')

        const blueSigHead = document.querySelectorAll('.blue #e-stat-body_x5F__x5F_head_value')

        const redSigBody = document.querySelectorAll('.red #e-stat-body_x5F__x5F_body_value')
        const blueSigBody = document.querySelectorAll('.blue #e-stat-body_x5F__x5F_body_value')

        const redSigLeg = document.querySelectorAll('.red #e-stat-body_x5F__x5F_leg_value')

        const blueSigLeg = document.querySelectorAll('.blue #e-stat-body_x5F__x5F_leg_value')
        ...
        const redHeadPercent = document.querySelectorAll('.red #e-stat-body_x5F__x5F_head_percent')
        const blueHeadPercent = document.querySelectorAll('.blue #e-stat-body_x5F__x5F_head_percent')

        const redBodyPercent = document.querySelectorAll('.red #e-stat-body_x5F__x5F_body_percent')
        const blueBodyPercent = document.querySelectorAll('.blue #e-stat-body_x5F__x5F_body_percent')

        const redLegPercent = document.querySelectorAll('.red #e-stat-body_x5F__x5F_leg_percent')
        const blueLegPercent = document.querySelectorAll('.blue #e-stat-body_x5F__x5F_leg_percent')

        return {
          redFightOverview: Array.from(redFightOverview, (n) => n.textContent.trim()),
          blueFightOverview: Array.from(blueFightOverview, (n) => n.textContent.trim()),
      ...
      function buildAllSections(data) {
        ...
        const zipFight = (raw, pct, att) =>
          raw.map((v, i) => ({
            landed: Number(v),
            percent: pct[i] ? parseFloat(pct[i].replace(/[()%]/g, '')) / PERCENT_MULTIPLIER : null,
            attempted: att[i] ? parseInt(att[i].replace(/\D/g, '') || '0', DECIMAL_RADIX) : null
          }))
        ...
        const zipTarget = (h, b, l, hp, bp, lp) =>
          h.map((_, i) => ({
            head: {
              landed: Number(h[i]),
              percent: hp[i] ? parseFloat(hp[i].replace(/[()%]/g, '')) / PERCENT_MULTIPLIER : null
            },
            body: {
              landed: Number(b[i]),
              percent: bp[i] ? parseFloat(bp[i].replace(/[()%]/g, '')) / PERCENT_MULTIPLIER : null
            },
            leg: {
              landed: Number(l[i]),
              percent: lp[i] ? parseFloat(lp[i].replace(/[()%]/g, '')) / PERCENT_MULTIPLIER : null
            }
          }))

        const totalRounds = data.redFightOverview.length / (FIGHT_STATS_CHUNK_SIZE + POSITION_STATS_CHUNK_SIZE)

        const zipRed = zipFight(data.redFightOverview, data.redPercent, data.redAttempted)
        const zipBlue = zipFight(data.blueFightOverview, data.bluePercent, data.blueAttempted)
        ...
        const wrap = (blocks, mapper) => ({
          ...
          rounds: blocks.slice(1).map((b, i) => ({ round: i + 1, ...mapper(b) }))
        })
